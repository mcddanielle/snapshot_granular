...
#################################################################
#################################################################
def plot_particles(xp,yp,radius,ax,Sx=60.0,Sy=60.0,i=0):
    '''
    ...
    colors1 = ['red', 'cornflowerblue']

    scatter1=ax.scatter(xp,yp,c=colors1[i],s=radius*radius*size,edgecolor='k')
...
if __name__ == "__main__":
    ...
    fig = plt.figure(figsize=(4.0*columns,4.3*rows),num=None, facecolor='w', edgecolor='k')
    ...
    for d,ax in zip([1.0,2.0],[ax1,ax2]):
        ...
        x_particle = np.zeros(n_particles)
        y_particle = np.zeros(n_particles)
        radius = np.zeros(n_particles)

        N_per_row = np.sqrt(n_particles) #square only
        ...
        for j in range(n_rows):
            ...
            color_int = int(d)-1
            plot_particles(x_particle,y_particle,radius,ax,Sx=60.0,Sy=60.0,i=color_int)
